...
def get_word_ratio(rdd1, word):
    ...
    for record in tdd1.collect():
        logger.info("__TSA__: The word " + word + " occurred " + str(record[1]) + " times in Sentiments of label " + str(record[0]))
...
def main():
    input_filename = raw_input("Please enter the filename (with extension)"
                               " that you want to test. OR enter 'D' for the default data set: ")
    if input_filename.lower() == "d":
        input_filename = "data/small.csv"
